...
@section('content')
    <div class="max-w-3xl">
        <div class="flex items-end justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-secondary-900 dark:text-secondary-50">{{ app()->getLocale() === 'ar' ? 'إضافة طريقة دفع' : 'Create Payment Method' }}</h1>
            </div>
            <a class="btn-outline" href="{{ route('admin.payment-methods.index') }}">{{ app()->getLocale() === 'ar' ? 'رجوع' : 'Back' }}</a>
        </div>

        <form class="mt-8 admin-card p-6 space-y-4" method="POST" action="{{ route('admin.payment-methods.store') }}">
            @csrf
            ...
            <div>
                ...
                <input class="form-input" name="icon" value="{{ old('icon') }}" placeholder="💳 🧾 💵 📱" />
            ...
            <div>
                ...
                <select class="form-input" name="type" required>
                    <option value="cod" @selected(old('type', 'cod') === 'cod')>{{ app()->getLocale() === 'ar' ? 'الدفع عند الاستلام' : 'Cash on Delivery' }}</option>
                    <option value="card" @selected(old('type') === 'card')>{{ app()->getLocale() === 'ar' ? 'بطاقة/فيزا' : 'Card/Visa' }}</option>
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <textarea class="form-input" rows="3" name="description_ar">{{ old('description_ar') }}</textarea>
                ...
                <div>
                    ...
                    <textarea class="form-input" rows="3" name="description_en">{{ old('description_en') }}</textarea>
