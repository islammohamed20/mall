...
@section('content')
    <div class="max-w-3xl">
        <div class="flex items-end justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-secondary-900 dark:text-secondary-50">{{ app()->getLocale() === 'ar' ? 'إضافة سلايدر' : 'Create Slider' }}</h1>
            </div>
            <a class="btn-outline" href="{{ route('admin.sliders.index') }}">{{ app()->getLocale() === 'ar' ? 'رجوع' : 'Back' }}</a>
        </div>

        <form class="mt-8 admin-card p-6 space-y-4" method="POST" action="{{ route('admin.sliders.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="form-label">{{ app()->getLocale() === 'ar' ? 'العنوان عربي' : 'Title (AR)' }}</label>
                    <input class="form-input" name="title_ar" value="{{ old('title_ar') }}" />
                ...
                <div>
                    ...
                    <input class="form-input" name="title_en" value="{{ old('title_en') }}" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input class="form-input" name="subtitle_ar" value="{{ old('subtitle_ar') }}" />
                ...
                <div>
                    ...
                    <input class="form-input" name="subtitle_en" value="{{ old('subtitle_en') }}" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input class="form-input" type="date" name="start_date" value="{{ old('start_date') }}" />
                </div>
                <div>
