...
@section('content')
    <div class="max-w-4xl">
        <div class="flex items-end justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-secondary-900 dark:text-secondary-50">{{ app()->getLocale() === 'ar' ? 'تعديل فعالية' : 'Edit Event' }}</h1>
                <div class="mt-2 text-sm text-secondary-600 dark:text-secondary-300">#{{ $event->id }}</div>
            </div>
            <a class="btn-outline" href="{{ route('admin.events.index') }}">{{ app()->getLocale() === 'ar' ? 'رجوع' : 'Back' }}</a>
        </div>

        <form class="mt-8 admin-card p-6 space-y-6" method="POST" action="{{ route('admin.events.update', $event) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select class="form-input" name="shop_id">
                    ...
                    @foreach ($shops as $shop)
                        <option value="{{ $shop->id }}" @selected((string) old('shop_id', $event->shop_id) === (string) $shop->id)>{{ $shop->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input class="form-input" name="title_ar" value="{{ old('title_ar', $event->title_ar) }}" required />
                ...
                <div>
                    ...
                    <input class="form-input" name="title_en" value="{{ old('title_en', $event->title_en) }}" required />
            ...
            <div>
                ...
                <input class="form-input" name="slug" value="{{ old('slug', $event->slug) }}" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input class="form-input" name="short_ar" value="{{ old('short_ar', $event->short_ar) }}" />
                </div>
                <div>
                    <label class="form-label">{{ app()->getLocale() === 'ar' ? 'مختصر English' : 'Short (EN)' }}</label>
                    <input class="form-input" name="short_en" value="{{ old('short_en', $event->short_en) }}" />
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <textarea class="form-input" rows="6" name="content_ar">{{ old('content_ar', $event->content_ar) }}</textarea>
                </div>
                <div>
                    <label class="form-label">{{ app()->getLocale() === 'ar' ? 'المحتوى English' : 'Content (EN)' }}</label>
                    <textarea class="form-input" rows="6" name="content_en">{{ old('content_en', $event->content_en) }}</textarea>
                </div>
            </div>
            ...
            <div>
                ...
                @if ($event->banner_url)
                    <div class="mt-3 rounded-xl overflow-hidden bg-gray-100 dark:bg-secondary-900 aspect-[4/2]">
                        <img class="w-full h-full object-cover" src="{{ $event->banner_url }}" alt="" loading="lazy" />
                    </div>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input class="form-input" type="date" name="start_date" value="{{ old('start_date', $event->start_date?->format('Y-m-d')) }}" required />
                ...
                <div>
                    ...
                    <input class="form-input" type="date" name="end_date" value="{{ old('end_date', $event->end_date?->format('Y-m-d')) }}" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input class="form-input" type="time" name="start_time" value="{{ old('start_time', $event->start_time) }}" />
                ...
                <div>
                    ...
                    <input class="form-input" type="time" name="end_time" value="{{ old('end_time', $event->end_time) }}" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input class="form-input" name="location_ar" value="{{ old('location_ar', $event->location_ar) }}" />
                </div>
                <div>
                    <label class="form-label">{{ app()->getLocale() === 'ar' ? 'الموقع English' : 'Location (EN)' }}</label>
                    <input class="form-input" name="location_en" value="{{ old('location_en', $event->location_en) }}" />
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input class="form-input" type="number" min="0" name="sort_order" value="{{ old('sort_order', $event->sort_order) }}" />
                </div>
                <div class="flex items-center gap-2 pt-7">
                    <input type="hidden" name="is_active" value="0" />
                    <input class="rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-secondary-700 dark:bg-secondary-950" type="checkbox" name="is_active" value="1" @checked(old('is_active', $event->is_active)) />
                    <span class="text-sm text-secondary-700 dark:text-secondary-200">{{ app()->getLocale() === 'ar' ? 'نشط' : 'Active' }}</span>
                </div>
                <div class="flex items-center gap-2 pt-7">
                    <input type="hidden" name="is_featured" value="0" />
                    <input class="rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-secondary-700 dark:bg-secondary-950" type="checkbox" name="is_featured" value="1" @checked(old('is_featured', $event->is_featured)) />
