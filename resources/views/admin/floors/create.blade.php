...
@section('content')
    <div class="max-w-3xl">
        <div class="flex items-end justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-secondary-900 dark:text-secondary-50">{{ app()->getLocale() === 'ar' ? 'إضافة دور' : 'Create Floor' }}</h1>
            </div>
            <a class="btn-outline" href="{{ route('admin.floors.index') }}">{{ app()->getLocale() === 'ar' ? 'رجوع' : 'Back' }}</a>
        </div>

        <form class="mt-8 admin-card p-6 space-y-4" method="POST" action="{{ route('admin.floors.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div>
                ...
                <input class="form-input" name="code" value="{{ old('code') }}" placeholder="e.g. GF, L1, L2" />
