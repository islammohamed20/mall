...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">تعديل المنشور</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">تعديل المنشور قبل النشر على فيسبوك</p>
        </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.facebook-posts.outgoing.update', $post) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <select name="shop_id" id="shop_id" required
                    ...
                    @foreach($shops as $shop)
                        <option value="{{ $shop->id }}" {{ old('shop_id', $post->shop_id) == $shop->id ? 'selected' : '' }}>
                            {{ $shop->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    <label class="relative flex items-center justify-center p-4 border-2 rounded-lg cursor-pointer transition"
                        ...
                        <input type="radio" name="post_type" value="text" x-model="postType" class="sr-only">
                    ...
                    <label class="relative flex items-center justify-center p-4 border-2 rounded-lg cursor-pointer transition"
                        ...
                        <input type="radio" name="post_type" value="photo" x-model="postType" class="sr-only">
                        <div class="text-center">
                    ...
                    <label class="relative flex items-center justify-center p-4 border-2 rounded-lg cursor-pointer transition"
                        ...
                        <input type="radio" name="post_type" value="link" x-model="postType" class="sr-only">
                        <div class="text-center">
            ...
            <div>
                ...
                <textarea name="message" id="message" rows="5"
                          ...
                          placeholder="اكتب نص المنشور هنا...">{{ old('message', $post->message) }}</textarea>
            ...
            @if($post->image)
                <div x-show="postType === 'photo'" x-cloak>
                    ...
                    <div class="rounded-lg overflow-hidden border border-gray-200 dark:border-secondary-700 inline-block">
                        <img src="{{ $post->image_url }}" alt="" class="max-h-48 object-contain">
                    </div>
                </div>
            @endif
            ...
            <div x-show="postType === 'photo'" x-cloak>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    {{ $post->image ? 'تغيير الصورة' : 'الصورة *' }}
                </label>
                <div class="border-2 border-dashed border-gray-300 dark:border-secondary-600 rounded-lg p-6 text-center hover:border-blue-400 transition"
                    ...
                    <template x-if="!imagePreview">
                        <label for="image" class="cursor-pointer">
                            ...
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500">انقر لرفع صورة جديدة</p>
            ...
            <div x-show="postType === 'link'" x-cloak>
                ...
                <input type="url" name="link_url" id="link_url" value="{{ old('link_url', $post->link_url) }}"
...
function postForm() {
    return {
        postType: '{{ old('post_type', $post->post_type) }}',
        imagePreview: null,
        charCount: {{ strlen($post->message ?? '') }},
