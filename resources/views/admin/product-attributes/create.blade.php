...
@section('content')
    <div class="max-w-3xl">
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-secondary-900 dark:text-secondary-50">{{ app()->getLocale() === 'ar' ? 'إضافة خاصية' : 'Create Attribute' }}</h1>
                <p class="mt-2 text-secondary-700 dark:text-secondary-200">{{ app()->getLocale() === 'ar' ? 'تعريف خاصية جديدة يمكن ربطها بتصنيفات المحلات.' : 'Define a new attribute and link it to shop categories.' }}</p>
            </div>
            <a class="btn-outline" href="{{ route('admin.product-attributes.index') }}">{{ app()->getLocale() === 'ar' ? 'رجوع' : 'Back' }}</a>
        </div>

        <form class="mt-8 admin-card p-6 space-y-6" method="POST" action="{{ route('admin.product-attributes.store') }}">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select class="form-input" name="input_type" required>
                        <option value="text" @selected(old('input_type') === 'text')>Text</option>
                        <option value="select" @selected(old('input_type') === 'select')>Select</option>
                        <option value="checkbox" @selected(old('input_type') === 'checkbox')>Checkbox</option>
                        <option value="multi_select" @selected(old('input_type') === 'multi_select')>Multi select</option>
                    </select>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea class="form-input" rows="4" name="options">{{ old('options') }}</textarea>
                ...
                <div>
                    ...
                    <input class="form-input" name="validation_rules" value="{{ old('validation_rules') }}">
