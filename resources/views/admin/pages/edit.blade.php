...
@section('content')
    <div class="max-w-4xl">
        <div class="flex items-end justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-secondary-900 dark:text-secondary-50">{{ app()->getLocale() === 'ar' ? 'تعديل الصفحة' : 'Edit Page' }}</h1>
            </div>
            <a class="btn-outline" href="{{ route('admin.pages.index') }}">{{ app()->getLocale() === 'ar' ? 'رجوع' : 'Back' }}</a>
        </div>

        <form class="mt-8 admin-card p-6 space-y-4" method="POST" action="{{ route('admin.pages.update', $page) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input class="form-input" name="slug" value="{{ old('slug', $page->slug) }}" placeholder="auto-generated" />
            ...
            <div>
                ...
                <textarea class="form-input" rows="8" name="content_ar">{{ old('content_ar', $page->content_ar) }}</textarea>
            ...
            <div>
                ...
                <textarea class="form-input" rows="8" name="content_en">{{ old('content_en', $page->content_en) }}</textarea>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <textarea class="form-input" rows="2" name="meta_description_ar">{{ old('meta_description_ar', $page->meta_description_ar) }}</textarea>
                ...
                <div>
                    ...
                    <textarea class="form-input" rows="2" name="meta_description_en">{{ old('meta_description_en', $page->meta_description_en) }}</textarea>
            ...
            <div>
                ...
                @if ($page->featured_image)
                    <div class="mb-2">
                        <img class="h-20 w-32 object-cover rounded" src="{{ asset('storage/' . $page->featured_image) }}" alt="">
                    </div>
                @endif
