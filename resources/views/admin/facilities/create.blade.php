...
@section('content')
    <div class="max-w-3xl">
        <div class="flex items-end justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-secondary-900 dark:text-secondary-50">{{ app()->getLocale() === 'ar' ? 'إضافة مرفق' : 'Create Facility' }}</h1>
            </div>
            <a class="btn-outline" href="{{ route('admin.facilities.index') }}">{{ app()->getLocale() === 'ar' ? 'رجوع' : 'Back' }}</a>
        </div>

        <form class="mt-8 admin-card p-6 space-y-4" method="POST" action="{{ route('admin.facilities.store') }}">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <textarea class="form-input" rows="3" name="short_ar">{{ old('short_ar') }}</textarea>
                ...
                <div>
                    ...
                    <textarea class="form-input" rows="3" name="short_en">{{ old('short_en') }}</textarea>
