...
تم تعزيز أمان التطبيق من خلال الإجراءات التالية:

- **رؤوس الأمان (Security Headers)**: إضافة Middleware عالمي يفرض HSTS، منع XSS، ومنع الـ Clickjacking (X-Frame-Options).
- **تأمين الكوكيز**: جميع الكوكيز الحساسة (بما فيها معرف الزائر) تم ضبطها لتكون `HttpOnly` لمنع الوصول إليها عبر JavaScript.
- **تأمين الجلسات**: ضبط الجلسات لتعمل عبر HTTPS فقط في بيئة الإنتاج.
- **تقييد CORS**: تقييد الوصول إلى الـ API من النطاقات الموثوقة فقط.
- **تدوير السجلات (Log Rotation)**: ضبط النظام لتسجيل السجلات بشكل يومي `daily` لمنع تضخم حجم الملفات.

## تحسين محركات البحث SEO

- **Sitemap ديناميكي**: تم إضافة ملف `sitemap.xml` يتم تحديثه تلقائياً ليشمل كافة المحلات والعروض والفعاليات النشطة.
- **Robots.txt**: مهيأ للسماح لمحركات البحث بالأرشفة مع الإشارة لخريطة الموقع.
